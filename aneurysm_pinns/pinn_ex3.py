# -*- coding: utf-8 -*-
"""
Corrected and Enhanced PINN Implementation with Nested Training Loop, Fixed Initialization, and Progress Bars
"""
...
print_every = 50  # Align with Code 1
...
batch_mesh = 500  # Increased batch size for mesh data
batch_cfd = 250   # Increased batch size for CFD data
...
params_filename = f'{save_fn}_params3.json'
...
def init_weights(m):
    ...
    if isinstance(m, nn.Linear):
        # Handle unsupported nonlinearity by mapping 'silu' to 'relu'
        if activation_function == 'silu':
            nonlinearity = 'relu'  # 'silu' is not supported, so we use 'relu'
        else:
            nonlinearity = activation_function
        try:
            nn.init.kaiming_normal_(m.weight, nonlinearity=nonlinearity)
        except ValueError as e:
            print(f"Initialization error for layer {m}: {e}")
            nn.init.kaiming_normal_(m.weight, nonlinearity='relu')  # Fallback to 'relu'
...
if load_previous:
    model, optimizer, scheduler, loaded_params = load_model(f'{save_fn}3.pt', model, optimizer, scheduler)
    if loaded_params is not None:
        print(f"Loaded training parameters from {load_fn}3.pt")
...
for epoch in tqdm(range(1, epochs + 1), desc="Epochs"):
    ...
    for mesh_batch in tqdm(mesh_loader, desc=f"Epoch {epoch}", leave=False):
    ...
    if avg_val_loss < best_val_loss:
        ...
        patience_counter = 0
        # Save the best model
        save_model(f'{save_fn}_best3.pt', model, optimizer, scheduler, training_params)
    else:
        patience_counter += 1
    ...
    if epoch % save_every == 0:
        save_model(f'{save_fn}_epoch_{epoch}3.pt', model, optimizer, scheduler, training_params)
        print(f"Model saved to {save_fn}_epoch_{epoch}3.pt")

print('Training phase complete.')

###################################### SAVE THE TRAINED MODEL
if save_end:
    save_model(f'{save_fn}3.pt', model, optimizer, scheduler, training_params)
    print(f"Model saved to {save_fn}3.pt")
...
try:
    model, _, _, _ = load_model(f'{save_fn}_best3.pt', model)
    print(f"Loaded the best model from {save_fn}_best3.pt")
except FileNotFoundError:
    print(f"Best model file '{save_fn}_best3.pt' not found. Proceeding with current model.")
...
fig.suptitle(f'Pressure and Velocity Field Comparison: Case {sc}3', fontsize=20)
plt.savefig(f'./plots/fields_comparison_case_{sc}3.png')
plt.show()

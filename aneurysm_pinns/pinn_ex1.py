# -*- coding: utf-8 -*-
"""
Updated PINN Implementation with Fixed DataLoader and Fully Connected Neural Network
"""
...
units_per_layer = 128  # Number of neurons per layer
...
batch_mesh = 500  # Batch size for mesh data
batch_cfd = 250   # Batch size for CFD data
...
for epoch in range(1, epochs + 1):
    ...
    if avg_val_loss < best_val_loss:
        ...
        patience_counter = 0
        # Save the best model
        save_model(f'{save_fn}_best1.pt', model, optimizer, scheduler, training_params)
    else:
        patience_counter += 1
...
model, _, _, _ = load_model(f'{save_fn}_best1.pt', model)
model.eval()
...
plt.savefig(f'fields_comparison_case_{sc}.png')
plt.show()

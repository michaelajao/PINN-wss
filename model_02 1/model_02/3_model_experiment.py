# pinn_resnet_densenet.py

# -*- coding: utf-8 -*-
"""
Enhanced PINN Implementation with ResNet and DenseNet Architectures,
Min-Max Normalization, Fixed Tensor Concatenation, Single Backward Pass,
Comprehensive Full Dataset Visualization, and Memory Management Enhancements.
"""
...
###################################### Neural Network Definitions with ResNet and DenseNet Structures

class ResidualBlock(nn.Module):
    """
    A Residual Block for ResNet-like architecture in fully connected networks.
    """
    def __init__(self, units_per_layer, activation):
        super(ResidualBlock, self).__init__()
        self.fc1 = nn.Linear(units_per_layer, units_per_layer)
...
class DenseBlock(nn.Module):
    """
    A Dense Block for DenseNet-like architecture in fully connected networks.
    """
    def __init__(self, in_features, growth_rate, activation):
...
class ResNetPINN(nn.Module):
    """
    ResNet-like Neural Network for PINN.
    """
    def __init__(
        self,
        input_size=3,
        ...
        self.input_layer = nn.Linear(input_size, units_per_layer)
        self.input_activation = self.activation

        self.blocks = nn.ModuleList([ResidualBlock(units_per_layer, self.activation) for _ in range(num_layers - 2)])
...
class DenseNetPINN(nn.Module):
    """
    DenseNet-like Neural Network for PINN.
    """
    def __init__(
        self,
        input_size=3,
        ...
        super(DenseNetPINN, self).__init__()
        self.activation = self.get_activation_function(activation)
        ...
        self.blocks = nn.ModuleList([
            DenseBlock(units_per_layer + growth_rate * i, growth_rate, self.activation)
            for i in range(num_layers - 2)
        ])

        self.output_layer = nn.Linear(units_per_layer + growth_rate * (num_layers - 2), output_size)
...
# Model architecture: 'fully_connected', 'resnet', or 'densenet'
model_architecture = "fully_connected"  # Change to 'fully_connected' or 'densenet' as needed
growth_rate = 32  # Only used for DenseNet
...
def initialize_model():
    """
    Initializes the neural network based on the selected architecture.
    """
    if model_architecture == "fully_connected":
